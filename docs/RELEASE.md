...
- GitHub Actions are pinned to immutable commits. The regular gate runs hygiene,
  Go tests/vet/vulnerability checks, Android unit tests, lint, and a minified
  candidate. After that gate is accepted, the tagged workflow rebuilds the
  native archive, signs the APK, and verifies the pinned certificate without
  printing secret material; it does not repeat the long Android JVM suite.
...
Before every version tag, the ordinary release gate must pass: source hygiene,
native test/vet/vulnerability checks, all Android JVM tests, both lint variants,
and a minified candidate. The protected tag workflow repeats source/native
hygiene and native checks, performs a fresh arm64 native rebuild, builds the
minified distribution, and matches its single signing certificate against the
repository pin; it does not duplicate the Android JVM and lint tasks. The
separate API-30 managed-device workflow must also pass the sparse-v1 database
migration before a tag is published.
