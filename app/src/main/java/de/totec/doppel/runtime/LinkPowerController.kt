    ...
    suspend fun run() = coroutineScope {
        ...
        val watchers =
            listOf(
                launch {
                    feed.requests
                        .onStart { subscribed[0].complete(Unit) }
                        .collect { request ->
                            remember(request)
                            poked.trySend(Unit)
                        }
        ...
        // Nothing is planned before both sources are live. The request channel queues an
        // earlier wake (including the tap that created this service); waiting here also
        // closes the smaller race where a new request lands between initial planning and
        // the request collector becoming active.
        subscribed.awaitAll()
        try {
