...
- Made **Send code** recover a terminally parked native bridge instead of issuing
  ...
  taps are now single-flight rather than queued as multiple reconnects. Pairing
  also buys one bounded listening window when LOW mode or quiet hours had put the
  bridge to sleep, including when that tap first creates the service; the normal
  power policy resumes automatically when the window ends.
...
- Stopped the signing workflow from running the same long Android JVM tests and
  lint twice after the main release gate. The tag job still repeats source and
  native verification, rebuilds the native archive, assembles the minified
  signed APK, pins its certificate, and publishes its checksum.
